...
const HeroPage = () => {
  ...
  // 1️⃣ Extract unique categories (remove duplicates)
  const uniqueCategories = useMemo(() => {
    if (!products) return [];

    const used = new Set(); // track category names
    return products
      .map((p) => p.category)
      .filter((cat) => {
        if (used.has(cat.name)) return false;
        used.add(cat.name);
        return true;
      });
  }, [products]);

  // 2️⃣ Filter featured products for hero carousel
  const featuredProducts = useMemo(() => {
    return products?.filter((p) => p.is_featured && p.preview) || [];
  }, [products]);
